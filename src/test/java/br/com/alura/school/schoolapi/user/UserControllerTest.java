...
class UserControllerTest {
    ...
    void should_retrieve_user_by_username() throws Exception {
        userRepository.save(new User("joana", "joana@email.com"));

        mockMvc.perform(get("/users/joana")
                        .accept(MediaType.APPLICATION_JSON))
                .andExpect(status().isOk())
                .andExpect(content().contentType(MediaType.APPLICATION_JSON))
                .andExpect(jsonPath("$.username", is("joana")))
                .andExpect(jsonPath("$.email", is("joana@email.com")));
    }
    ...
    void should_add_new_user() throws Exception {
        NewUserRequest newUser = new NewUserRequest("alessandro", "alessandro@email.com");

        mockMvc.perform(post("/users")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(jsonMapper.writeValueAsString(newUser)))
                .andExpect(status().isCreated())
                .andExpect(header().string("Location", "/users/alessandro"));
    }
    ...
    void should_not_allow_duplication_of_username() throws Exception {
        userRepository.save(new User("mariana", "mariana@email.com"));

        NewUserRequest newUser = new NewUserRequest("mariana", "mary@alura.com.br");
